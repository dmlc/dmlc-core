...
using enable_constructor_from_value = enable_if_t<std::is_constructible<T, U &&>::value
...
#pragma GCC diagnostic pop
  ...
  explicit optional(
      enable_if_t<std::is_constructible<T, Args...>::value, in_place_t>, Args &&...args) {
  ...
  explicit optional(
      enable_if_t<std::is_constructible<T, std::initializer_list<U> &, Args &&...>::value,
          in_place_t>,
  ...
  template <typename U = value_type, enable_if_t<std::is_convertible<U &&, T>::value> * = nullptr,
  ...
  template <typename U, enable_constructor_from_other<T, U, const U &> * = nullptr,
...
std::istream &operator>>(std::istream &is, optional<T> &t) {
  ...
  is.read(buf, 4);
  if (is.fail() || buf[0] != 'N' || buf[1] != 'o' || buf[2] != 'n' || buf[3] != 'e') {
    is.clear();
    is.seekg(origin);
    T x;
    is >> x;
