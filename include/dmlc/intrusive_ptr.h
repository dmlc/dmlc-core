 ...
 *     IntrusivePtrRefCount(RefCountedData const *t) noexcept {
 *       return t->ref;
 *     }
 *   };
 *
 *   auto p = make_intrusive<RefCountedData>();
 *
 * \endcode
 */
...
// Constructs an object of type T and wraps it in a IntrusivePtr.
template <typename T, typename... Args>
IntrusivePtr<T> make_intrusive(Args&&... args) { // NOLINT
...
namespace std {
template <class T>
void swap(dmlc::IntrusivePtr<T> &x,  // NOLINT
          dmlc::IntrusivePtr<T> &y) noexcept {
  x.swap(y);
}

template <typename T>
struct hash<dmlc::IntrusivePtr<T>> : public dmlc::IntrusivePtr<T>::Hash {};
}      // namespace std
