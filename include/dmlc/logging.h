...
inline std::string Demangle(const char *msg_str) {
  using std::string;
  string msg(msg_str);
...
inline std::string StackTrace(
    size_t start_frame = 1, const size_t stack_size = DMLC_LOG_STACK_TRACE_SIZE) {
  using std::string;
...
DEFINE_CHECK_FUNC(_NE, !=)
    ...
    #define CHECK(x) \
      if (!(x))      \
      dmlc::LogMessageFatal(__FILE__, __LINE__).stream() << "Check failed: " #x << ": "
