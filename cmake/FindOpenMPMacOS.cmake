...
# Patch libdmlc.dylib so that it depends on @rpath/libomp.dylib instead of
# /opt/homebrew/opt/libomp/lib/libomp.dylib or other hard-coded paths.
# Doing so enables dmlc-core to interoperate with multiple kinds of OpenMP
...
function(patch_openmp_path_macos target target_default_output_name)
  ...
  if(NOT BUILD_SHARED_LIBS)
    message(STATUS "${CMAKE_CURRENT_FUNCTION}() has no effect when building static lib")
    return()
  endif()
  ...
  # Get the name of the dmlc-core lib, e.g. libdmlc
  get_target_property(
    __LIBDMLC_OUTPUT_NAME
    ${target}
    OUTPUT_NAME
  )
  if(NOT __LIBDMLC_OUTPUT_NAME)
    set(__LIBDMLC_OUTPUT_NAME "${target_default_output_name}")
  endif()

  # Get the file name of the dmlc-core lib, e.g. libdmlc.dylib
  if(CMAKE_SHARED_LIBRARY_SUFFIX_CXX)
    set(
      __LIBDMLC_FILENAME_${target} "${__LIBDMLC_OUTPUT_NAME}${CMAKE_SHARED_LIBRARY_SUFFIX_CXX}"
      CACHE INTERNAL "Shared library filename ${target}"
    )
  else()
    set(
      __LIBDMLC_FILENAME_${target} "${__LIBDMLC_OUTPUT_NAME}.dylib"
      CACHE INTERNAL "Shared library filename ${target}"
    )
  endif()

  message(STATUS "Creating shared lib for target ${target}: ${__LIBDMLC_FILENAME_${target}}")
