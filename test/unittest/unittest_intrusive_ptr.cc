...
TEST(IntrusivePtr, Basic) {
  IntrusivePtr<ForIntrusivePtrTest> ptr = make_intrusive<ForIntrusivePtrTest>();
  auto p = ptr.get();
...
TEST(IntrusivePtr, MakeIntrusive) {
  auto p = make_intrusive<ForIntrusivePtrTest>(1.0f, 2);
  ASSERT_EQ(p->data, 3.0f);
  ASSERT_EQ(p.use_count(), 1);

#if defined(_MSC_VER) && _MSC_VER < 1910
  // Error when calling `new ForIntrusivePtrTest(NotCopyConstructible(1.0f))` on MSVC
  // 2015:
  //
  //   unknown file: error: SEH exception with code 0xc0000005 thrown in the test body.
  //
  std::cout << "\x1B[33mWarning\u001B[0m: "
            << "Skipping IntrusivePtr.MakeIntrusive on MSVC < 2017." << std::endl;
  return;
#endif

  p = make_intrusive<ForIntrusivePtrTest>(NotCopyConstructible(1.0f));
  ASSERT_EQ(p->data, 1.0f);
  ASSERT_EQ(p.use_count(), 1);
  ASSERT_EQ(IntrusivePtrRefCount(p.get()).Count(), 1);
}
} // namespace dmlc
