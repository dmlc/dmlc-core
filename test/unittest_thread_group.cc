...
TEST(ThreadGroup, ThreadLaunchAutoRemove) {
  ...
  for (int x = 0; x < 200; ++x) {
    dmlc::ThreadGroup::Thread::SharedPtr thread = std::make_shared<dmlc::ThreadGroup::Thread>(
        std::string("test_thread_ar ") + std::to_string(x), thread_group.get());
...
TEST(ThreadGroup, ThreadLaunchAutoRemoveWithDelay) {
  ...
  for (int x = 0; x < 200; ++x) {
    dmlc::ThreadGroup::Thread::SharedPtr thread = std::make_shared<dmlc::ThreadGroup::Thread>(
        std::string("test_thread_rwd ") + std::to_string(x), thread_group.get());
...
TEST(ThreadGroup, ThreadLaunchNoAutoRemove) {
  ...
  for (int x = 0; x < 200; ++x) {
    dmlc::ThreadGroup::Thread::SharedPtr thread = std::make_shared<dmlc::ThreadGroup::Thread>(
        std::string("test_thread_nao ") + std::to_string(x), thread_group.get());
...
TEST(ThreadGroup, ThreadLaunchNoAutoRemoveWithDelay) {
  ...
  for (int x = 0; x < 200; ++x) {
    dmlc::ThreadGroup::Thread::SharedPtr thread = std::make_shared<dmlc::ThreadGroup::Thread>(
        std::string("test_thread_narwd ") + std::to_string(x), thread_group.get());
...
TEST(ThreadGroup, ThreadLaunchQueueThread) {
  ...
  std::shared_ptr<BQ> queue_thread
      = std::make_shared<BQ>("BlockingQueueThread", thread_group.get());
  ...
  BQ::launch_run(queue_thread,
      ...
      [queue_thread](int item) -> int {
        std::cout << "ITEM: " << item << std::endl << std::flush;
        if (item >= 2 && item <= 3) {
  ...
  dmlc::TimerThread<Duration>::start(
      timer_thread, Duration(TIMER_PERIOD), [timer_thread, start_time, &count]() -> int {
        if ((count + 1) % 5 == 0) {
          ...
          std::cout << "[" << (count + 1) << "] TIME: " << GetDurationInMilliseconds(start_time)
                    << "\n";
        }
        ++count;
        return 0;  // return 0 means continue
...
TEST(ThreadGroup, TimerThreadSimple) {
  ...
  size_t count = 0;
  // Launch the queue thread, passing queue item handler as lambda
  dmlc::CreateTimer("TimerThreadSimple", Duration(TIMER_PERIOD), thread_group.get(),
